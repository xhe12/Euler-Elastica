...
%%
    ...
    eta = eps*(a+b*(BDx(n1)+BDy(n2)).^2)./(pi*(eps^2+phi.^2)) + lambda21.*n1 ...
          + lambda22.*n2;
...
%     A_absp = sqrt(AP1.^2 + AP2.^2);
    ...
    coef = 2*b*eps*absp./(pi*(eps^2+phi.^2));
%     coef = 2*b*eps*A_absp./(pi*(eps^2+phi.^2));
    c = max(coef(:));
    Dmax = max(r2*absp(:).^2);
...
%         n2old1 = n2;
        ...
        a11 = -c * Fg1 + Dmax;
        a12 = -c * Fg2;
        vv = (c - coef).*(BDx(n1)+BDy(n2));
        f1 = (r2*p1 - lambda21).*absp + (Dmax- r2*absp.^2).*n1 - FDx(vv);
        Ff1 = fft2(f1);

        a21 = -c * Fg3;
        a22 = -c * Fg4 + Dmax;
        f2 = (r2*p2 - lambda22).*absp + (Dmax - r2*absp.^2).*n2- FDy(vv);
        Ff2 = fft2(f2);
...
%         tol = sum(sqrt((n1(:) - n1old1(:)).^2 + (n2(:) - n2old1(:)).^2))...
%             ./sum(sqrt(n1old1(:).^2 + n2old1(:).^2));
% 
%             
%         i = i+1;
% 
...
%     dif1 = -FDx(coef.*(BDx(n1)+BDy(n2))) + (r2*(absp.*n1-p1)+lambda21).*absp;
%     dif2 = -FDy(coef.*(BDx(n1)+BDy(n2))) + (r2*(absp.*n2-p2)+lambda22).*absp;
...
%     mesh(dif_R12)
    ...
    R = (a + (BDx(Ra1)+BDy(Ra2)).^2*b).*eps./(pi*(eps^2+phi.^2)).*absp;
...
figure(4)
plot(x,log(L1(1:maxit)),'r', x,log(L2(1:maxit)),'g',x,log(L3(1:maxit)),'b')
legend('L1','L2','L3')

figure(5)
plot(x,log(R1(1:maxit)),'r', x,log(R2(1:maxit)),'g',...
        x,log(abs(R3(1:maxit))),'b')
legend('Residue1','Residue2','Residue3')
...
imshow(f)
hold on
contour(phi,[-.01,.01],'r','LineWidth',1.5)
%  title(['b = ',num2str(b),' r1= ',num2str(r1),' r2 = ',num2str(r2),...
%      ' r3 = ', num2str(r3),' iteration = ', num2str(1000)])
hold off

% figure(9)
% contour(phi,[-.01,.01],'r','LineWidth',3)
